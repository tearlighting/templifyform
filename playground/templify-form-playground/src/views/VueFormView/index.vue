  ...
  <div class="size-full flex justify-center items-center text-text">
    <div class="w-full min-w-100 max-w-150 md:px-2 md:py-3">
      <templify-form :template="<any>formTemplate" :form-data="formData">
        <template #password="{ item }">
          <ElFormItem :prop="item.prop" :error="item.error.resolve({ t })" :class="item.formItemClassName">
            ...
            <ElInput size="large" v-model="formData[item.prop as keyof typeof formData]" :class="item.formItemContentClassName" show-password> </ElInput>
          </ElFormItem>
        </template>
      </templify-form>
      <div class="mb-5">
      ...
      <div class="mb-5">
        ...
        <el-button size="large" type="primary" @click="() => setError('code', 'defaultForm.customError')">setError</el-button>
